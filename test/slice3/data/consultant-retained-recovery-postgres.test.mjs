...
(database ? test : test.skip)(
  "MB-UX-LIVE-001 L11 retained recovery preserves history, lineage and atomic zero-cost publication",
  async () => {
    ...
    try {
      ...
      await pool.query(
        "INSERT INTO account(account_id,display_name,status) VALUES($1,'L11 isolated recovery test','active')",
        [identity.account_id],
      );
      ...
      await pool.query(
        `INSERT INTO consultant_research_round(round_id,account_id,user_profile_id,run_id,classification_id,round_number,plan,status,execution_id,output,continuation)
         VALUES($1,$2,$3,$4,$5,1,$6,'completed',$7,$8,$9)`,
        [
          sourceRoundId,
          identity.account_id,
      ...
      function build(saved, executionId) {
        buildCalls++;
        assert.deepEqual(saved.output, source);
