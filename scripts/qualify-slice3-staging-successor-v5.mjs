...
const AUTHORIZATION_ID =
  "OWNER-SLICE3-STAGING-LIVE-QUALIFICATION-2026-08-30-V5";
const EXECUTION_SIGNAL = "I_AUTHORIZE_SYNTHETIC_STAGING_QUALIFICATION_V5";
const AUTHORIZATION_SHA256 =
  "5B82371AE53858560B506F71E29F9B4302AE4304DDF444DD54FA623D9DADF783";
const PRODUCTION_POLICY_SHA256 =
  "46FCAF0C2D2B66F8BAB8526C48E448A24B2E9F65B065AAA99135CA6AF048DB23";
const PREDECESSOR_SESSION_ID = "v6-C7F3C97A9B2845E26B17F0D4";
const PREDECESSOR_LEDGER_SHA256 =
  "51D929CBC3184A14C9C1D4CE5AFEBF374C999352C378B50EAE88AB1A6BC39A1E";
...
const SESSION_ID = "v6-A41D735D225C80F1B7916D62";
const MAX_EXTERNAL_HTTP_CALLS = 50;
const MAX_MODEL_POSTS = 2;
const MAX_COST_USD = 99.7720095;
const PRIOR_MODEL_POSTS = 11;
const PRIOR_COST_USD = 0.2279905;

const DEFAULTS = Object.freeze({
  authorizationPath: resolve(
    "governance/slice3-staging-live-qualification-successor-authorization.v5.json",
  ),
  productionPolicyPath: resolve("config/slice3/research-route-policy.v1.json"),
  credentialPath: resolve("APIKeys.md"),
  executorPath: resolve("scripts/qualify-slice3-staging-successor-v5.mjs"),
...
function validateAuthorization(value) {
  if (
    value?.schemaVersion !==
      "matchbase.slice3-staging-live-qualification-successor-authorization/v5" ||
    ...
    value.predecessor?.costConsumedUsd !== 0.009663 ||
    value.predecessor?.cumulativeProviderModelPosts !== 11 ||
    value.predecessor?.cumulativeCostUsd !== 0.1806455 ||
    ...
    value.credentialGate?.failureClass !== "QUOTA_UNPROVEN" ||
    value.credentialGate?.accountCreditsObservedUsd !== 10 ||
    value.credentialGate?.proceedToBoundedModelCapabilityProof !== true ||
    value.ownerCeiling?.maximumProviderCalls !== 50 ||
    value.ownerCeiling?.maximumCostUsd !== 100 ||
    value.ownerCeiling?.remainingProviderCallsBeforeV5 !== 39 ||
    value.ownerCeiling?.remainingCostUsdBeforeV5 !== MAX_COST_USD ||
    ...
    value.openRouterRequestPolicy?.zdr !== true ||
    value.openRouterRequestPolicy?.dataCollection !== "deny" ||
    value.openRouterRequestPolicy?.requireParameters !== true ||
    value.openRouterRequestPolicy?.allowFallbacks !== false ||
    JSON.stringify(value.openRouterRequestPolicy?.providerOnly) !==
      JSON.stringify(["google-vertex"]) ||
    JSON.stringify(value.openRouterRequestPolicy?.providerOrder) !==
      JSON.stringify(["google-vertex"]) ||
    value.directGeminiRequestPolicy?.providerNativeSearchRequired !== true ||
    value.directGeminiRequestPolicy?.minimumNonemptySearchQueries !== 1 ||
    value.directGeminiRequestPolicy?.maximumAcceptedSearchQueries !== 12 ||
    value.directGeminiRequestPolicy?.answerFromMemoryProhibited !== true ||
    value.directGeminiRequestPolicy?.groundingSupportRequired !== true ||
    value.outputContractRemediation?.fixtureIdAuthority !== "server_owned" ||
    JSON.stringify(value.outputContractRemediation?.providerOutputKeys) !==
      JSON.stringify(["answer", "sourceSummary"]) ||
    value.outputContractRemediation?.providerFixtureIdProhibited !== true ||
    value.outputContractRemediation?.schemaDescriptionsRequired !== true ||
    value.outputContractRemediation?.rawPayloadPersistence !== false ||
    value.outputContractRemediation?.cumulativeProviderModelPostsBeforeV5 !==
      PRIOR_MODEL_POSTS ||
    value.outputContractRemediation?.cumulativeConservativeCostUsdBeforeV5 !==
      PRIOR_COST_USD ||
    value.outputContractRemediation?.singleAllocationConservativeReserveUsd !==
      qualificationWorstCaseCostUsd() ||
...
export async function assessSuccessor(overrides = {}) {
  ...
  return Object.freeze({
    schemaVersion:
      "matchbase.slice3-staging-live-qualification-successor-preflight/v5",
    disposition:
...
function sanitizedFailure(error, routePath, counters) {
  ...
  return Object.freeze({
    schemaVersion:
      "matchbase.slice3-staging-live-qualification-successor-failure/v5",
...
export async function executeSuccessor({
  ...
  await writeJsonExclusive(join(stateDirectory, "00-authorization.json"), {
    schemaVersion:
      "matchbase.slice3-staging-live-qualification-successor-session/v5",
    authorizationId: AUTHORIZATION_ID,
    sessionId: SESSION_ID,
    predecessorSessionId: PREDECESSOR_SESSION_ID,
    syntheticOnly: true,
  ...
  for (
    ...
    await writeJsonExclusive(
      ...
      {
        schemaVersion:
          "matchbase.slice3-staging-live-qualification-successor-reservation/v5",
        authorizationId: AUTHORIZATION_ID,
        sessionId: SESSION_ID,
        callNumber: index + 1,
  ...
  await writeJsonExclusive(join(stateDirectory, "05-terminal-summary.json"), {
    schemaVersion:
      "matchbase.slice3-staging-live-qualification-successor-terminal/v5",
    authorizationId: AUTHORIZATION_ID,
    sessionId: SESSION_ID,
  ...
  if (failures.length > 0) {
    ...
    return Object.freeze({
      schemaVersion:
        "matchbase.slice3-staging-live-qualification-successor-execution/v5",
      disposition: "FAIL",
      authorizationId: AUTHORIZATION_ID,
      sessionId: SESSION_ID,
...
export const SLICE3_STAGING_SUCCESSOR_V5_CONSTANTS = Object.freeze({
  authorizationId: AUTHORIZATION_ID,
