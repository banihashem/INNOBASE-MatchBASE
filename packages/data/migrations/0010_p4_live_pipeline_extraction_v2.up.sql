...
ALTER TABLE live_research_execution_reservation
  ...
  CHECK (
    pipeline_identity_record IS NULL OR (
      ...
      pipeline_identity_record->>'extractionVersion' IN (
        'untrusted-source-boundary.v1',
        'untrusted-source-boundary.v2'
      ) AND
...
COMMENT ON CONSTRAINT live_research_pipeline_identity_closed
  ...
  'Closed live pipeline identity; v1 and visible-text v2 extraction identities are accepted without rewriting historical rows.';
