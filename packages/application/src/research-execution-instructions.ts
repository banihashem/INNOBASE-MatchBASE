...
PRICE DISCOVERY: Actively seek public prices matching the requested product/service, grade, route, market, currency, unit and trade basis. Prefer source-dated prices under seven days old, then under thirty days old. Public supplier offers and external market benchmarks are different evidence; never attribute a benchmark to a supplier or infer a price date from retrieval time. Missing current prices remain explicit gaps, never invented amounts or grounds to suppress an otherwise supported supplier. A dedicated priced price-search task is scheduled separately by the application; do not execute it inside this supplier-discovery call.
...
export type NativeResearchPhase = `discovery_${string}` | "verification";
