...
const AUTHORIZATION_ID =
  "OWNER-SLICE3-STAGING-LIVE-QUALIFICATION-2026-08-30-V4";
const EXECUTION_SIGNAL = "I_AUTHORIZE_SYNTHETIC_STAGING_QUALIFICATION_V4";
const AUTHORIZATION_SHA256 =
  "0838A79491F25A20E61B9EA4FD4ECB2398E26D6E7D55E9290DCDB789DC685AC6";
const PRODUCTION_POLICY_SHA256 =
  "46FCAF0C2D2B66F8BAB8526C48E448A24B2E9F65B065AAA99135CA6AF048DB23";
const PREDECESSOR_SESSION_ID = "v6-6E2E91B1E51DAA84C42A19F3";
const PREDECESSOR_LEDGER_SHA256 =
  "81940679BE1782C3953ADA1F598AC82B6C39F5D81F26AFFC0ADF0026BE2A48E4";
...
const SESSION_ID = "v6-C7F3C97A9B2845E26B17F0D4";
const MAX_EXTERNAL_HTTP_CALLS = 50;
const MAX_MODEL_POSTS = 2;
const MAX_COST_USD = 99.8290175;
const PRIOR_MODEL_POSTS = 9;
const PRIOR_COST_USD = 0.1709825;

const DEFAULTS = Object.freeze({
  authorizationPath: resolve(
    "governance/slice3-staging-live-qualification-successor-authorization.v4.json",
  ),
  productionPolicyPath: resolve("config/slice3/research-route-policy.v1.json"),
  credentialPath: resolve("APIKeys.md"),
  executorPath: resolve("scripts/qualify-slice3-staging-successor-v4.mjs"),
...
function validateAuthorization(value) {
  if (
    value?.schemaVersion !==
      "matchbase.slice3-staging-live-qualification-successor-authorization/v4" ||
    ...
    value.predecessor?.costConsumedUsd !== 0.02348 ||
    value.predecessor?.cumulativeProviderModelPosts !== 5 ||
    value.predecessor?.cumulativeCostUsd !== 0.0569665 ||
    ...
    value.ownerCeiling?.remainingProviderCallsBeforeV4 !== 41 ||
    value.ownerCeiling?.remainingCostUsdBeforeV4 !== MAX_COST_USD ||
    ...
    value.directGeminiRequestPolicy?.searchQueriesRequired !== 1 ||
    value.directGeminiRequestPolicy?.answerFromMemoryProhibited !== true ||
    value.outputSchemaRemediation?.fixtureDiscriminator !==
      "type_plus_single_value_enum" ||
    value.outputSchemaRemediation?.rawPayloadPersistence !== false ||
    value.outputSchemaRemediation?.diagnosticModelPostsConsumed !== 4 ||
    value.outputSchemaRemediation?.diagnosticConservativeCostUsd !== 0.114016 ||
    value.outputSchemaRemediation?.cumulativeProviderModelPostsBeforeV4 !==
      PRIOR_MODEL_POSTS ||
    value.outputSchemaRemediation?.cumulativeConservativeCostUsdBeforeV4 !==
      PRIOR_COST_USD ||
...
export async function assessSuccessor(overrides = {}) {
  ...
  return Object.freeze({
    schemaVersion:
      "matchbase.slice3-staging-live-qualification-successor-preflight/v4",
    disposition:
...
function sanitizedFailure(error, routePath, counters) {
  ...
  return Object.freeze({
    schemaVersion:
      "matchbase.slice3-staging-live-qualification-successor-failure/v4",
...
export async function executeSuccessor({
  ...
  await writeJsonExclusive(join(stateDirectory, "00-authorization.json"), {
    schemaVersion:
      "matchbase.slice3-staging-live-qualification-successor-session/v4",
    authorizationId: AUTHORIZATION_ID,
    sessionId: SESSION_ID,
    predecessorSessionId: PREDECESSOR_SESSION_ID,
    syntheticOnly: true,
  ...
  for (
    ...
    await writeJsonExclusive(
      ...
      {
        schemaVersion:
          "matchbase.slice3-staging-live-qualification-successor-reservation/v4",
        authorizationId: AUTHORIZATION_ID,
        sessionId: SESSION_ID,
        callNumber: index + 1,
  ...
  await writeJsonExclusive(join(stateDirectory, "05-terminal-summary.json"), {
    schemaVersion:
      "matchbase.slice3-staging-live-qualification-successor-terminal/v4",
    authorizationId: AUTHORIZATION_ID,
    sessionId: SESSION_ID,
  ...
  if (failures.length > 0) {
    ...
    return Object.freeze({
      schemaVersion:
        "matchbase.slice3-staging-live-qualification-successor-execution/v4",
      disposition: "FAIL",
      authorizationId: AUTHORIZATION_ID,
      sessionId: SESSION_ID,
...
export const SLICE3_STAGING_SUCCESSOR_V4_CONSTANTS = Object.freeze({
  authorizationId: AUTHORIZATION_ID,
