...
if ($Checkpoint -in @("Canary", "Cutover")) {
  ...
  for($i=0;$i-lt $expectedRanges.Count;$i++){ $mainPriority=1000+$i; $mainExpression="inIpRange(origin.ip, '$($expectedRanges[$i])') && request.headers['host'].lower() == '$($migration.Hostname)'"; $mainMatched=@($armor.rules|Where-Object{$_.priority -eq $mainPriority -and $_.action -ceq "allow" -and $_.match.expr.expression -ceq $mainExpression}); if($mainMatched.Count-ne 1){throw "Main Cloud Armor closed priority/range/host proof failed."} };$canaryIpExpression=($expectedRanges|ForEach-Object{"inIpRange(origin.ip, '$_')"})-join" || ";$canaryExpression="($canaryIpExpression) && request.headers['host'].lower() == '$($migration.CanaryHostname)'";$canaryBand=@($armor.rules|Where-Object{$_.priority-ge2000-and$_.priority-le2014});$canaryMatched=@($canaryBand|Where-Object{$_.priority-eq2000-and$_.action-ceq"allow"-and$_.match.expr.expression-ceq$canaryExpression});if($canaryMatched.Count-ne1-or$canaryBand.Count-ne1){throw "Canary Cloud Armor consolidated closed range/host proof failed."}
