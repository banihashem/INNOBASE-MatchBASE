...
if [ "${MATCHBASE_RUNTIME_KIND:-}" = "worker" ]; then
  case "${MATCHBASE_ROUTE_POLICY_SHA256:-}" in
    ????????????????????????????????????????????????????????????????) ;;
    *) echo "Worker runtime requires a route-policy SHA-256 identity." >&2; exit 78 ;;
  esac
  case "${MATCHBASE_ROUTE_POLICY_SHA256}" in
    *[!0-9a-f]*) echo "Worker route-policy identity is not lowercase hexadecimal." >&2; exit 78 ;;
  esac
  route_policy_path=/app/config/slice3/research-route-policy.v1.json
  if [ ! -f "$route_policy_path" ]; then
    echo "Worker governed route policy is missing." >&2
    exit 78
  fi
  ...
  if [ "$actual_route_policy_sha256" != "$MATCHBASE_ROUTE_POLICY_SHA256" ]; then
    echo "Worker governed route-policy bytes do not match the deployment identity." >&2
    exit 78
  fi
  ...
  if [ "$policy_environment" != "$MATCHBASE_DEPLOYMENT_ENVIRONMENT" ] || [ "$policy_version" != "${MATCHBASE_ROUTE_POLICY_VERSION:-}" ]; then
    echo "Worker route policy environment or version does not match the deployment identity." >&2
    exit 78
  fi
fi
exec "$@"
