...
const AUTHORIZATION_ID =
  "OWNER-SLICE3-STAGING-LIVE-QUALIFICATION-2026-08-30-V2";
const EXECUTION_SIGNAL = "I_AUTHORIZE_SYNTHETIC_STAGING_QUALIFICATION_V2";
...
const PREDECESSOR_SESSION_ID = "v6-40CB8BEE95ABACB012107300";
const PREDECESSOR_LEDGER_SHA256 =
  "DA4F6075133A448C021B49E4FF0CE520FF62FBA0A61D3B64744F4B2CB4504588";
const SESSION_ID = "v6-4FA7336D74F38010CBEE9D66";
const MAX_EXTERNAL_HTTP_CALLS = 50;
const MAX_MODEL_POSTS = 2;
const MAX_COST_USD = 99.988513;
const PRIOR_MODEL_POSTS = 1;
const PRIOR_COST_USD = 0.011487;

const DEFAULTS = Object.freeze({
  authorizationPath: resolve(
    "governance/slice3-staging-live-qualification-successor-authorization.v2.json",
  ),
  productionPolicyPath: resolve("config/slice3/research-route-policy.v1.json"),
  credentialPath: resolve("APIKeys.md"),
  executorPath: resolve("scripts/qualify-slice3-staging-successor-v2.mjs"),
...
function validateAuthorization(value) {
  if (
    value?.schemaVersion !==
      "matchbase.slice3-staging-live-qualification-successor-authorization/v2" ||
    ...
    value.predecessor?.reasonCode !== "QUALIFICATION_SEARCH_GROUNDING_FAILED" ||
    value.predecessor?.providerModelPostsConsumed !== PRIOR_MODEL_POSTS ||
    value.predecessor?.costConsumedUsd !== PRIOR_COST_USD ||
    value.ownerCeiling?.maximumProviderCalls !== 50 ||
    value.ownerCeiling?.maximumCostUsd !== 100 ||
    value.ownerCeiling?.remainingProviderCallsBeforeV2 !== 49 ||
    value.ownerCeiling?.remainingCostUsdBeforeV2 !== MAX_COST_USD ||
...
export async function assessSuccessor(overrides = {}) {
  ...
  return Object.freeze({
    schemaVersion:
      "matchbase.slice3-staging-live-qualification-successor-preflight/v2",
    disposition:
...
export async function executeSuccessor({
  ...
  if (failures.length > 0) {
    ...
    return Object.freeze({
      schemaVersion:
        "matchbase.slice3-staging-live-qualification-successor-execution/v2",
      disposition: "FAIL",
      authorizationId: AUTHORIZATION_ID,
      sessionId: SESSION_ID,
...
export const SLICE3_STAGING_SUCCESSOR_V2_CONSTANTS = Object.freeze({
  authorizationId: AUTHORIZATION_ID,
