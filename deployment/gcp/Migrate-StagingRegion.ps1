...
if (Test-Checkpoint -Name "Canary") {
  ...
  Write-Output "Prerequisite: apply Configure-StagingCanaryEdge.ps1, then produce signed Canary evidence proving Armor priorities 2000-2014, dual-host ACTIVE certificate, HTTPS proxy attachment, proxied DNS with public A/AAAA, Full strict, and redacted origin-admission transform identity."
  ...
  $canaryPathMatcher = @("compute", "url-maps", "add-path-matcher", [string]$migration.UrlMap, "--project=$ProjectId", "--global", "--path-matcher-name=$canaryMatcher", "--default-service=$($migration.CanaryBackendService)", "--quiet")
  ...
  if ($Apply) {
    ...
    if (-not (@($urlMapBefore.pathMatchers | Where-Object name -ceq $canaryMatcher).Count)) { $null = Invoke-MigrationGcloud -Arguments $canaryPathMatcher }
    if (-not (@($urlMapBefore.hostRules | Where-Object { $_.hosts -ccontains $migration.CanaryHostname -and $_.pathMatcher -ceq $canaryMatcher }).Count)) { $null = Invoke-MigrationGcloud -Arguments $canaryHostRule }
